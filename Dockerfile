...
# docker run -d --name sugarcrm-mysql -e MYSQL_ROOT_PASSWORD=1233456 mysql
...
WORKDIR /sugar
RUN apt-get update
RUN apt-get install -y wget unzip apache2 libapache2-mod-php5 php5-curl php5-gd php5-imap php5-json php5-mysqlnd
...
RUN bash -c "chown www-data.www-data crm/{config.php,config_override.php,sugarcrm.log}"
